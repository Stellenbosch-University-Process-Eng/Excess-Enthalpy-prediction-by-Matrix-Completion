...
def generate_stan_model_code(include_clusters: bool, add_zeros: bool, refT: bool):
    model_code = '''
        functions {
    ...
    if refT:
        model_code += '''
            vector<lower=0>[N_T] sigma_refT;    // reference temperature'''
    model_code += '''
        }
    ...
    if add_zeros:
        model_code += '''
                x2[N_T*N_C+2*(i-1)+1] = 0;
                x2[N_T*N_C+2*(i-1)+2] = 1;
                T2[N_T*N_C+2*(i-1)+1] = T2_int[i];
                T2[N_T*N_C+2*(i-1)+2] = T2_int[i];'''
    model_code += '''
            } 
        ...
        parameters {'''
    if include_clusters and not refT:
        model_code += '''
            array[N_T,M] matrix[D,K] U_raw_means;   // mean feature matrices U
            array[N_T,M] matrix[D,K] V_raw_means;   // mean feature matrices V'''
    elif include_clusters and refT:
        model_code += '''
            array[M] matrix[D,K] U_raw_means;       // mean feature matrices U
            array[M] matrix[D,K] V_raw_means;       // mean feature matrices V'''
    if refT:
        model_code += '''
            array[M] matrix[D,N] U_raw_refT;        // reference temperature feature matrices U
            array[M] matrix[D,N] V_raw_refT;        // reference temperature feature matrices V'''
    model_code += '''
    ...
    if include_clusters and not refT:
        ...
        model_code += '''
                    U[t,m] = U_raw[t,m] * sigma_refT[t] + (U_raw_refT[m] .* sigma_cluster + U_raw_means[m] * C);
                    V[t,m] = V_raw[t,m] * sigma_refT[t] + (V_raw_refT[m] .* sigma_cluster + V_raw_means[m] * C);'''
    elif not include_clusters and refT:
        model_code += '''
                    U[t,m] = U_raw[t,m] * sigma_refT[t] + U_raw_refT[m];
                    V[t,m] = V_raw[t,m] * sigma_refT[t] + V_raw_refT[m];'''
    elif not include_clusters and not refT:
        model_code += '''
                    U[t,m] = U_raw[t,m];
                    V[t,m] = V_raw[t,m];'''
    model_code += ''' 
    ...
    if include_clusters and not refT:
        ...
        model_code += '''
            array_1D_mat_to_array(U_raw_means) ~ std_normal();
            array_1D_mat_to_array(V_raw_means) ~ std_normal();
            array_1D_mat_to_array(U_raw_refT) ~ std_normal();
            array_1D_mat_to_array(V_raw_refT) ~ std_normal();'''
    if not include_clusters and refT:
        model_code += '''
            array_1D_mat_to_array(U_raw_refT) ~ std_normal();
            array_1D_mat_to_array(V_raw_refT) ~ std_normal();'''
    ...
    if add_zeros:
        model_code += '''
                            y_0 += dot_product(U[t,m,:,Idx_known[i,1]], V[t,m,:,Idx_known[i,1]])^2;
                            y_1 += dot_product(U[t,m,:,Idx_known[i,2]], V[t,m,:,Idx_known[i,2]])^2;'''
    model_code += '''
                        }'''
    if add_zeros:
        model_code += '''
                        y_MC_pred[N_C*N_T+2*(t-1)+1] = y_0;
                        y_MC_pred[N_C*N_T+2*(t-1)+2] = y_1;'''
    model_code += '''
                    }
    ...
    if add_zeros:
        model_code += '''
                            y_0 += dot_product(U[t,m,:,Idx_unknown[i,1]], V[t,m,:,Idx_unknown[i,1]])^2;
                            y_1 += dot_product(U[t,m,:,Idx_unknown[i,2]], V[t,m,:,Idx_unknown[i,2]])^2;'''
    model_code += '''
                        }'''
    if add_zeros:
        model_code += '''
...
for include_clusters in c:
    for add_zeros in z:
        for refT in t:
            model_code = generate_stan_model_code(include_clusters=include_clusters, add_zeros=add_zeros, refT=refT)
            stan_file = f'{path}/Pure_PMF_include_clusters_{include_clusters}_zeros_{add_zeros}_refT_{refT}.stan'
